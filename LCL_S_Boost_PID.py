    ...
    for j in range(Loop):
        ...
        for i in range(int(Inner_Time / TimeGap)):
            ...
            if (count in count_percent) and (count != 0):
                ...
                temp_result[j]['UCP'] = XBox[3, last_i:].tolist()
                temp_result[j]['UCT'] = XBox[4, last_i:].tolist()
                temp_result[j]['UCS'] = XBox[5, last_i:].tolist()
                temp_result[j]['Ur'] = XBox[6, last_i:].tolist()
                temp_result[j]['Uinv'] = XBox[7, last_i:].tolist()
                temp_result[j]['Iout'] = XBox[8, last_i:].tolist()
                temp_result[j]['Vout'] = XBox[9, last_i:].tolist()
                temp_result[j]['Vlp'] = (XBox[1, last_i:] * w * LP).tolist()
                temp_result[j]['ICP'] = (XBox[3, last_i:] * w * CP).tolist()
                temp_result[j]['VLT'] = (XBox[0, last_i:] * w * LT).tolist()
                temp_result[j]['VLR'] = (XBox[2, last_i:] * w * LS).tolist()
